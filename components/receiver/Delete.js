...
import { deleteDataFromIndexDB } from "@/lib/Database";
...
const Delete = ({ message, id, data }) => {
    ...
    const deleteYesClick =  async  () => {
        try {
            const msg = await deleteDataFromIndexDB('receiver', id);
            message(msg);
        } catch (error) {
            console.log(error);
                    ...
                    <div className="w-11/12 md:w-1/2 mx-auto mb-10 bg-white border-2 border-gray-300 rounded-md shadow-md duration-300">
                        ...
                        <div className="p-4 lg:p-6 flex flex-col space-y-4">
                            <div className="w-full">
                                ...
                                <h1 className="text-center text-gray-600 font-bold">{identify}</h1>
                            </div>
